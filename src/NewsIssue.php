...
class NewsIssue {
	...
	public function loadFromHTML($sourceHTML) {
		...
		$result = preg_match('#<\\!-- subject="(.*)" -->#', 
			$this->sourceHTML, $matches);
		echo $result;
		if ($result < 1) {

			debugEmail('Error: Unable to parse news title', $this->sourceHTML); die ('Error: Unable to parse 
			news title');
		}
		$subject = $matches[1]; $this->issueTitle = 'Enhanced ' . preg_replace('/V[0-9]+ /', '', $subject);
		
		// Load issue month path
		$result = preg_match('#<\\!-- isoreceived="([0-9]{6})#', $this->sourceHTML, 
			$matches); $this->monthPathString = $matches[1];
		
		// Load email body
		$result = preg_match('/<\\/address>\\n<p>\\n(.+?) +([SMTWF].*)<p>End of ' . preg_quote($subject, '/') . '/s', $this->sourceHTML, $matches);
		if ($result < 1) {
			debugEmail('Error: Unable to parse news body', $this->sourceHTML);
			die ('Error: Unable to parse news body');
		}
